...
docker: test-assets
	...
	@docker run -v $(TMP):/buildpack:ro --network=host --log-driver=syslog --log-opt syslog-address=udp://logs7.papertrailapp.com:37558  --rm -it -e "GITHUB_TOKEN=$(GITHUB_TOKEN)" -e "GO_BUCKET_URL=$(GO_BUCKET_URL)" -e "IMAGE=$(IMAGE)" $(IMAGE) bash -c "cd /buildpack; $(BASH_COMMAND)"
	@rm -rf $(TMP)
